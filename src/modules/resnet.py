...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        if self.modal == "audio":
            x = self.conv1_a(x)
        elif self.modal in ["flow_IC", "flow_EN"]:
            x = self.conv1_flow(x)
        else:
            x = self.conv1(x)
        x = self.bn1(x)
...
def _resnet(arch, block, layers, pretrained, progress, modal, **kwargs):
    ...
    if pretrained:
        ...
        # set the conv1_flow weights and dimensions in the checkpoint according to the model used
        ...
        if modal == "flow_IC": 
            ...
            conv1_flow_wts = conv1_wts.mean(1).unsqueeze(1)                                
